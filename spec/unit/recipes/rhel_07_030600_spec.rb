...
# Check:
#
# Verify the operating system generates audit records when successful/unsuccessful account access count events occur.# # Check the file system rule in "/etc/audit/audit.rules" with the following commands:# # # grep -i /var/log/tallylog /etc/audit/audit.rules# # -w /var/log/tallylog -p wa -k logins# # If the command does not return any output, this is a finding.#
#
######
#
# Fix:
#
# Configure the operating system to generate audit records when successful/unsuccessful account access count events occur.# # Add or update the following rule in "/etc/audit/rules.d/audit.rules":# # -w /var/log/tallylog -p wa -k logins# # The audit daemon must be restarted for the changes to take effect.#
#
######

require 'spec_helper'

describe '::rhel_07_030600' do
