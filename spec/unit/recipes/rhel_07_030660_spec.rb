...
# Check:
#
# Verify the operating system generates audit records when successful/unsuccessful attempts to use the "chage" command occur.# # Check the file system rule in "/etc/audit/audit.rules" with the following command:# # # grep -i /usr/bin/chage /etc/audit/audit.rules# # -a always,exit -F path=/usr/bin/chage -F perm=x -F auid>=1000 -F auid!=4294967295 -k privileged-passwd# # If the command does not return any output, this is a finding.#
#
######
#
# Fix:
